...
\item Population genetics: infer demography, infer recombination, and infer selection.
...
Bioinformatics depends heavily on models providing simplified representations of biological complexity.  As such, there are often many alternative models for any objective, with varying numbers of parameters and assumptions.  It is tempting to believe that more parameterized models are more accurate, but they are also subject to overfitting, added computational complexity, and decreasing returns.  A guiding principle of \Rpackage{DECIPHER} is to follow the data, wherever it may take you.  Rather than blindly trust that more sophisticated models are better, function defaults in \Rpackage{DECIPHER} are determined by their performance on real data.  The preference for empirical benchmarks reflects a belief that only real data can provide a true reference point.  Simulated benchmarks, in contrast, often rely on related model assumptions and can result in circular affirmation of incorrect models.

There are many examples of following the data throughout the \Rpackage{DECIPHER} package.  For instance, \Rfunction{Treeline} uses the minimum evolution \Rfunarg{method} by default to construct phylogenetic trees.  Maximum likelihood is more popular and commonly assumed to be preferable since it is more sophisticated, but empirical data suggest minimum evolution results in more accurate trees on average.  \Rfunction{Treeline} can also generate maximum likelihood trees, but this is not the default \Rfunarg{method}.  In this way, \Rpackage{DECIPHER} is designed to provide good results by default and support alternative configurations if desired -- functions are designed to be both powerful and intuitive.
...
\ytl{2025}{\Rfunction{DetectRepeats} published \cite{Cho:2025} (\Rfunction{DetectRepeats})}
\ytl{2026}{\Rfunction{Treeline} method published \cite{Wright:2026} (\Rfunction{Treeline})}
\bigskip
\rule{\linewidth}{1pt}%
\end{minipage}%
\end{table}
...
\bibitem{Cho:2025}
{S-T Cho \& ES Wright}
\newblock Accurate detection of tandem repeats exposes ubiquitous reuse of biological sequences
\newblock {\em Nucleic Acids Research}, 53(17), gkaf866, 2025.
...
\newblock {\em The R Journal}, 16, 191-200, 2024.
