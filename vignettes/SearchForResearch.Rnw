%\VignetteIndexEntry{Searching Biological Sequences for Research}
...
%------------------------------------------------------------
\section{Controlling for false discoveries}
%------------------------------------------------------------
...
\subsection{Option 2: The knockoff approach for limiting false discoveries}
...
ranking <- ranking[cumsum(ranking > N) <= N*FDR]
hits <- hits[sort(ranking[ranking <= N]),] # filtered hits
nrow(hits) # fewer search hits after controlling FDR
@
...
\bibitem{Lu:2024}
{Lu, Y. Y.}, {Noble, W. S.}, \& {Keich, U.}
\newblock A BLAST from the past: revisiting blastp's \textit{E}-value.
\newblock {Bioinformatics}, 40(12), btae729. doi:10.1093/bioinformatics/btae729, 2024.
