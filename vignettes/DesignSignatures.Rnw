...
if (TYPE=="sequence") {
    ...
    colnames(signatures) <- abbreviate(ids, 10)
} else if (TYPE=="melt") {
...
if (TYPE=="sequence") {
    ...
    for (i in seq_along(ids)) {
        ...
        if (length(w) > 0)
            segments(i - 0.5, xaxs[w], i + 0.5, xaxs[w], lwd=2)
    }
}
@
