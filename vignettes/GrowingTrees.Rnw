%\VignetteIndexEntry{Growing Phylogenetic Trees in R with Treeline}
...
\title{Growing Phylogenetic Trees in R with Treeline}
...
\item Dense taxon sampling generally results in better trees by breaking up longer branches and helping to avoid saturation effects that spuriously attract long branches.  Adding diverse orthologs to the alignment is one of the most straightforward ways to dependably improve trees \cite{Zwickl:2002}.
...
\item Specifying a model of evolution is important for datasets that have undergone many mutations, because models account for repeated substitutions at the same site.  However, the specific choice of evolutionary model generally has less of an effect than the choice of optimization objective (i.e., minimum evolution).
\end{itemize}
...
For the next example, we will grow a maximum likelihood phylogenetic tree, which is the most computationally demanding optimization objective that is supported.  We will set a stringent time limit (0.01 hours) to make this example faster, although longer time limits (e.g., 24 hours) are advised because setting very short time limits leaves the result partly up to luck.  Again, note that setting a time limit negates the purpose of setting a seed, so omitting a time limit is required for reproducibility.
...
Each branch in a phylogenetic tree partitions the leaves into two sets.  We can use non-parametric bootstrapping to assign a confidence to that partitioning.  High bootstrap values imply the sequences reliably support a given partition, otherwise the existence of a branch is called into question.  The best way to quantify branch support is by using multiple loci, i.e., how often trees constructed from different sets of genes yield the same partition (see Section~\ref{sec:support}).  However, we can also bootstrap an individual alignment by resampling sites (columns) or blocks (groups of columns) of the alignment with replacement.

Many different single-branch methods have been developed for estimating local branch supports.  These include the aBayes probabilities \cite{Ecker:2024} that are automatically included on maximum likelihood trees output by \Rfunction{Treeline}.  Single-branch methods fail to account for the dependencies among surrounding branches that undermine the support of a branch.  For this reason, bootstrapping is preferable to obtain more precise estimates of branch support.  The main problem with bootstrapping is that it is slow.  Fast approximations have been developed, but these can be poorly calibrated to the actual bootstrap value at each branch.

Site-level bootstrapping has many different flavors, which is why it is not directly incorporated into the \Rfunction{Treeline} function as it is for multi-locus bootstrapping in the \Rfunction{Zipline} function.  The code below can be tailored to different applications, such as bootstrapping with non-uniform site sampling or different models of evolution.  Keeping bootstrapping outside the \Rfunction{Treeeline} function also facilitates the splitting of separate bootstrap replicates across multiple computers to accelerate the computation of support values.  The basic idea is to record how often a branch in the original tree was supported and then use that information to annotate the tree.

The code below implements a variant of bootstrapping called block bootstrapping.  If the block size is \code{1} then it is the same as the standard bootstrap, which assumes the data is independent and identically distributed.  This assumption is violated for sequencing data, because sites are linked and information is not evenly distributed across an alignment.  The solution is to use block bootstrapping, where blocks are sampled and then sites are sampled within each block.  Recent work suggests blocks that split the data directly in half provide better calibrated measures of branch support than standard (1 block) bootstrapping.  The number of blocks is set by the \code{blocks} variable below.

<<expr13,eval=TRUE,echo=TRUE>>=
reps <- 100 # number of bootstrap replicates (typically >= 100)
blocks <- 2 # positive integer
# 1 = standard bootstrap (~37% of sites unsampled per replicate)
# 2 = half splits (~47% of sites unsampled per replicate)
# 20 = many blocks (~52% of sites unsampled per replicate)
...
# generate blocks
L <- width(seqs)[1]
start <- unique(round(seq(0, L, length.out=blocks + 1)))
end <- start[-1]
...
for (i in seq_len(reps)) {
	# sample positions
	r <- integer(L) # randomly sampled positions
	count <- 0
	for (j in sample(blocks, replace=TRUE)) {
		b <- start[j]:end[j] # block positions
		r[count + seq_along(b)] <- b[sample(length(b), replace=TRUE)]
