...
\Rfunction{Treeline} supports many \Robject{MODELS} of evolution (Fig. \ref{f1}).  In many cases, these \Robject{MODELS} can be extended by appending the model with \code{"+F"}, \code{"+G\#"}, or \code{"+Indels"}.  Here is the list of built-in \Robject{MODELS}:
...
exclusive <- function(x) {
	if (!is.leaf(x)) { # leaves are trivially exclusive
		...
		if (all(max_dist < d[-leaves, leaves]))
			attr(x, "edgePar") <- list(col="purple")
	}
	x
}
...
If you are feeling down on your luck, you might want to double-check the inputs and outputs for any issues.  First, we can check for any input sequences with unexpected few or many characters by comparing character frequencies across all input sequences.  We can also look for input sequences that significantly deviate from the expected background frequencies using Pearson's chi-squared test.  Outliers may point to spurious sequences that should be double-checked for correctness or completion.
...
D <- DistanceMatrix(seqs, type="dist", verbose=FALSE, processors=1)
