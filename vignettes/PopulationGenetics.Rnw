...
The function provides maximum likelihood estimates of the transition/transversion ratio (kappa), expected substitution rate (theta), and $\omega$ in non-overlapping regions across the alignment.  The default behavior is to only calculate a single $\omega$ value for the whole alignment, but this will rarely be greater than \code{1} as negative selection typically overwhelms any signal of positive selection.  This is shown in the example below (Fig. \ref{f5}).

\begin{centerfig}
<<expr6,eval=TRUE,echo=TRUE,fig=TRUE,include=TRUE,height=6,width=6>>=
z <- InferSelection(dna, windowSize=NA, showPlot=TRUE)
z
@
\caption{\label{f5} Estimate of $\omega$ across the entire alignment.}
\end{centerfig}

Changing the \code{windowSize} to \code{1} will cause \Rfunction{InferSelection} to estimate $\omega$ at every codon.  This makes the analysis take longer but provides more useful information.  Statistically significant sites (or regions) with large values of $\omega$ (> 1) are possibly of biological interest, as these represent where the protein is evolving under positive selection.  It is particularly informative to apply this analysis to many different genes from the same population and rank the genes by the fraction (or number) of positively selected codons.
