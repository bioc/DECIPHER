	...
	if (useEmpirical) {
		if (xtype == 3L) {
			if (correctBackground) {
				...
				correction <- 0.07 # correction factor to standardize log-odds scores
			}
			residues <- c(A=0.079, R=-0.038, N=0.091, D=0.067, C=0.352, Q=-0.084, E=-0.097, G=0, H=-0.921, I=0.322, L=0.113, K=-0.121, M=-0.376, F=0.248, P=-0.188, S=-0.206, T=0.029, W=0.321, Y=0.246, V=0.255) # log-odds in tandem repeat
			...
			if (correctBackground) {
				correction <- 0.76 # correction factor to standardize log-odds scores
			} else {
				correction <- 0.33 # correction factor to standardize log-odds scores
			}
			periods <- c(1.276841, -5.489323, 3.788301, 0.003926434, 0.1902238, 5.464936) # sigmoid fit for log-odds of periodicity relative to background
		}
		lens <- c(NA_real_, -0.1, -2.2, 0.5, 0.3, 1.2, 1.9, 4.5, 2.4, 4.8) # log-odds of repeat copy number relative to background
		gapCost <- c(-3, -0.15) # gap open and extension coefficients
		...
		if (xtype == 3L) {
			...
			if (correctBackground) {
				correction <- 0.56 # correction factor to standardize log-odds scores
			} else {
